...
import { airbnbDB, userObjectStore, } from '../db/db' // 引入数据库和对象仓库

const storeName = Object.keys(userObjectStore)[0]
...
export async function userSignApi(params: any) {
  ...
  const hasMobile = await new Promise((resolve, reject) => {
    airbnbDB.getList(storeName).then((res: any) => {
      setTimeout(() => {
        loading.close()
      }, 200)
  ...
  if (hasMobile) {
    ...
    result = await new Promise((resolve, reject) => {
      airbnbDB.updateItem(storeName, params).then(res => {
        setTimeout(() => {
          loading.close()
        }, 200)
...
export async function userLogoutApi() {
  ...
  if (correct.code === '000005') {
    ...
    const params = await new Promise((resolve, reject) => {
      airbnbDB.getItem(storeName, correct.userId).then(res => {
        resolve(res)
      })
    })
